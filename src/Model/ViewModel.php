...
class ViewModel extends Model
{
        ...
        if (empty($this->trueTableName)) {
            ...
            foreach ($this->viewFields as $key => $view) {
                ...
                if (isset($view['_table'])) {
                    ...
                    // edit by tank 2017.09.15
                    $class = 'App\\Common\\Model\\' . $key . 'Model';
                    $model = new $class();
    ...
    protected function _options_filter(&$options)
    {
        if (isset($options['field']))
            $options['field'] = $this->checkFields($options['field']); else
    ...
    private function _checkFields($name, $fields)
    {
        ...
        if (false !== $pos = array_search('*', $fields)) {
            $model = new self($name);
            $fields = array_merge($fields, $model->getDbFields());
            unset($fields[$pos]);
        }
        return $fields;
    }
