...
class Cx extends TagLib
{
    ...
    public function _import($tag, $content, $isFile = false, $type = '')
    {
        ...
        if (isset($tag['value'])) {
            ...
            $name = $this->autoBuildVar($name);
            if (!empty($varArray))
                $name = $this->tpl->parseVarFunction($name, $varArray); else
                $name = 'isset(' . $name . ')';
    ...
    public function _for($tag, $content)
    {
        ...
        foreach ($tag as $key => $value) {
            $value = trim($value);
            if (':' == substr($value, 0, 1))
                $value = substr($value, 1); elseif ('$' == substr($value, 0, 1))
                $value = $this->autoBuildVar(substr($value, 1));
            switch ($key) {
                case 'start':
                    $start = $value;
